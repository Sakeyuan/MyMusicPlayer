#ifndef INTERNALBUTTON_H
#define INTERNALBUTTON_H
#include <QObject>
...
class InternalButton : public QPushButton
{
    Q_OBJECT
    Q_PROPERTY(bool self_enabled READ getSelfEnabled WRITE setSelfEnabled)              //  是否启用自定义按钮(true)
    Q_PROPERTY(bool parent_enabled READ getParentEnabled WRITE setParentEnabled)        //  是否启用父类按钮(false)
    Q_PROPERTY(bool fore_enabled READ getForeEnabled WRITE setForeEnabled)              //  是否绘制自定义按钮前景色
    ...
    Q_PROPERTY(QColor icon_color READ getIconColor WRITE setIconColor)                 //  前景图标颜色
    ...
    Q_PROPERTY(QColor hover_color READ getHoverColor WRITE setHoverColor)               //  鼠标悬停颜色
    Q_PROPERTY(QColor press_color READ getPressColor WRITE setPressColor)               //  鼠标按下颜色
    Q_PROPERTY(int hover_duration READ getHoverAniDuration WRITE setHoverAniDuration)         //  鼠标悬浮动动画周期
    Q_PROPERTY(int press_duration READ getPressAniDuration WRITE setPressAniDuration)         //  鼠标悬按下动画周期
    Q_PROPERTY(int click_duration READ getClickAniDuration WRITE setClickAniDuration)         //  鼠标悬点击动画周期
    ...
    Q_PROPERTY(bool text_dynamic_size READ getTextDynamicSize WRITE setTextDynamicSize)               //  修改字体大小时间调整按钮最小尺寸
    Q_PROPERTY(bool leave_after_clicked READ getLeaveAfterClick WRITE setLeaveAfterClick)               //  鼠标单击松开后取消悬浮效果
    Q_PROPERTY(bool show_animation READ getShowAni WRITE setShowAni)               //  是否启用出现动画（鼠标移开则消失）（false)
    Q_PROPERTY(bool water_animation READ getWaterRipple WRITE setWaterRipple)               //  是否启用点击水波动画(true)
    Q_PROPERTY(int font_size READ getFontSizeT WRITE setFontSizeT)                            //按钮字体动画效果

public:
    InternalButton(QWidget* parent = nullptr);
    InternalButton(QString text, QWidget* parent = nullptr);
    InternalButton(QIcon icon, QWidget* parent = nullptr);
    InternalButton(QPixmap pixmap, QWidget* parent = nullptr);
    InternalButton(QIcon icon,QString text, QWidget* parent = nullptr);
    InternalButton(QPixmap pixmap,QString text, QWidget* parent = nullptr);
    ~InternalButton(){}
    enum PaintModel{
        None,           //无前景，只使用背景
        Text,           // 纯文字(替代父类)
