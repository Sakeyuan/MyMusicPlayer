...
QCarousel::QCarousel(QWidget *parent) : QWidget(parent),currentIndex(0)
{
    ...
    connect(autoSlideTimer, &QTimer::timeout, this, &QCarousel::slideToRight);
}

void QCarousel::setPixmapSize(QSize size)
{
    this->oneSize = size;
...
void QCarousel::setPixmapScale(bool scale)
{
...
void QCarousel::setAutoSlide(int interval)
{
    if (interval)
    {
...
void QCarousel::addImage(const QPixmap &pixmap, QString text)
{
    insertImage(labels.size(), pixmap, text);
}

void QCarousel::insertImage(int index, const QPixmap &pixmap, QString text)
{
    // 图片
    ...
    // 指示球
    InternalButton* btn = new InternalButton(this);
    ...
    connect(btn, &InternalButton::signalMouseEnter, this, [=]{
...
void QCarousel::removeImage(int index)
{
...
void QCarousel::setCurrentIndex(int index)
{
...
int QCarousel::getCurrentIndex() const
{
    return currentIndex;
}

void QCarousel::slideToLeft()
{
    if (!labels.size())
        return ;
...
QPixmap QCarousel::getScaledRoundedPixmap(QPixmap pixmap) const
{
    // 缩放
...
void QCarousel::adjustLabels(SideHideLabel *leavingLabel)
{
    if (!labels.size())
        return ;
...
void QCarousel::moveTo(SideHideLabel *label, QRect geometry)
{
...
void QCarousel::resizeEvent(QResizeEvent *event)
{
...
bool QCarousel::eventFilter(QObject *obj, QEvent *event)
{
